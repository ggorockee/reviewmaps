...
### 4. 종료 시 광고: 리워드 광고 (Rewarded Ad) ✅ 기존 유지
**위치**: 뒤로가기 버튼으로 앱 종료 시도 시
- **유형**: 선택적 리워드 광고 (ExitRewardDialog)
- **정책 준수**: ✅ 사용자 선택권 제공, 강제하지 않음
- **파일**: `lib/screens/main_screen.dart:89-106`

**❌ 네이티브 광고로 변경 시 위험**:
- 앱 종료를 방해하는 것으로 간주될 수 있음
- Google/Apple 앱 리젝 위험 높음
- **결론**: 현재 리워드 광고 방식 유지 (정책상 안전)
...
- `lib/widgets/exit_reward_dialog.dart` - 종료 시 리워드 광고

### 화면 통합
- `lib/screens/splash_screen.dart` - 전면광고 표시
- `lib/screens/home_screen.dart` - 배너 + 네이티브 광고
- `lib/screens/main_screen.dart` - 종료 시 리워드 광고
