...
async def login(request, payload: UserLoginRequest):
    ...
    if not user.is_active:
        raise HttpError(403, "비활성화된 계정입니다.")
...
async def create_anonymous_session(request):
    ...
    - 60시간 유효기간의 세션 토큰 발급
    """
    session_token = create_anonymous_session_id()
...
async def convert_anonymous_to_user(request, payload: ConvertAnonymousRequest):
    ...
    if not session_id:
        raise HttpError(401, "유효하지 않은 세션 토큰입니다.")
...
@router.get("/me", response=UserResponse, summary="내 정보 조회")
