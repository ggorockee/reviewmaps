...
def create_keyword_alerts_on_campaign_save(sender, instance, created, **kwargs):
    ...
    logger.info(f"[Signal] 활성 키워드 수: {active_keywords.count()}")
    ...
    if alerts_to_create:
        ...
        logger.info(f"[Signal] {len(alerts_to_create)}개의 알림 생성 완료 - 캠페인 ID: {instance.id}")
    else:
        logger.info(f"[Signal] 매칭된 키워드 없음 - 캠페인 ID: {instance.id}, 제목: {campaign_title[:50]}")
